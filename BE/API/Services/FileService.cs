...
public interface IMinioFileService
{
    ...
    Task<ResDetailStatFile> GetStatFileAsync(string key, string bucket = MinioOptions.DefaultBucket);

    Task<IEnumerable<ResStatFile>> GetListObjectAsync(string bucket = MinioOptions.DefaultBucket);
...
public class MinioFileService : IMinioFileService
{
    ...
    public MinioFileService(MinioClient client)
    {
        ...
        var config = new MapperConfiguration(config => { config.AddProfile<FileProfile>(); });
    ...
    public async Task<ResDetailStatFile> GetStatFileAsync(string key, string bucket)
    {
        try
        {
            ...
            return _mapper.Map<ObjectStat, ResDetailStatFile>(rawStat);
        }
        catch (Exception ex)
        {
            throw;
    ...
    public async Task<IEnumerable<ResStatFile>> GetListObjectAsync(string bucket)
    {
        try
        {
            ...
            var rtn = new List<ResStatFile>();

            var items = _client.ListObjectsAsync(args);
            await items.ForEachAsync(item => { rtn.Add(_mapper.Map<Item, ResStatFile>(item)); });
